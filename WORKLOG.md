...
### A3 客户端 + 集成开发工具 自动登录（Win32 方案）

**需求：** 陛下在账套编辑中给客户端 + 开发工具分别配账套用户名 + 密码，启动账套时自动登录原生窗口。

**重要差异：** 之前做的 CDP 自动登录只对**网页版**（h5comerp 的 antd-mobile 表单）有效，原生 Win32 窗口（君则A3.exe / 君则A3集成开发工具.exe）必须走 Win32 API 自动填表。

**实现：**

1. **A3Tools.Common/Models/Account.cs**
   - 新增字段 `DevToolsPassword`（加密存储，注释「集成开发工具密码」）
   - 新增属性 `HasDevToolsAutoLogin`（用户名 + 开发工具密码都不空才返 true）

2. **A3Tools/Services/DataService.cs**
   - `LoadAccounts` 解密 DevToolsPassword
   - `SaveAccounts` 加密 DevToolsPassword（IsEncrypted 判断避免重复加密）
   - `UpdateAccount` 保留旧 DevToolsPassword（同 ServerUsername 逻辑）

3. **A3Tools/Forms/AccountDialog.Designer.cs**
   - 新增 Label + TextBox `lblDevToolsPassword` / `txtDevToolsPassword`（UseSystemPasswordChar=true）
   - 位置在「账套用户名」下面
   - lblWebHint 文案改为「🌐 自动登录配置（账套用户名=A3 客户端/开发工具/网页三处共用；客户端密码=账套密码；开发工具密码=单独加密存储）」
   - 窗体高度 1530 → 1590

4. **A3Tools/Forms/AccountDialog.cs**
   - LoadAccount：填 DevToolsPassword，Root 模式设 UseSystemPasswordChar=false 显示明文
   - GetAccount：保存 DevToolsPassword

5. **A3Tools/Services/Win32AutoLoginHelper.cs（新增）**
   - Win32 API 封装：FindWindow / EnumWindows / EnumChildWindows / GetWindowText / GetClassName / SendMessage / BM_CLICK 等
   - `LaunchAndAutoLogin(exePath, windowTitleContains, accountName, username, password, timeoutMs)`
     - 同步启动 exe（Process.Start）
     - 立即返回 Process（不阻塞调用方）
     - `Task.Run` 异步 TryAutoLoginAsync
   - `TryAutoLoginAsync`：轮询找登录窗 → 等 800ms → FillLoginFormAsync
   - `FillLoginFormAsync`：枚举子控件 → 按 Label 找兄弟 Edit → WM_SETTEXT 填值 → BM_CLICK 点确定
   - 日志写到 exe 同目录 `win32-login.log`（与 cdp.log 同风格）

6. **A3Tools/Forms/MainForm.cs**
   - 「启动电脑端」分支：检测 `ServerPassword + ServerUsername` 都非空 → 调 `Win32AutoLoginHelper.LaunchAndAutoLogin("君则A3.exe", "君则A3", accountName, ServerUsername, ServerPassword)`
   - 「启动开发工具」分支：检测 `DevToolsPassword + ServerUsername` 都非空 → 调 `Win32AutoLoginHelper.LaunchAndAutoLogin("君则A3集成开发工具.exe", "A3IDE", accountName, ServerUsername, DevToolsPassword)`
   - 不满足条件 → 旧逻辑 `Process.Start(UseShellExecute=true)`

**编译：** `dotnet build A3Tools.sln` 0 错误（2 NU1701 警告，无关）。

**待测试 / 已知隐患：**
- 登录窗标题匹配可能冲突：「君则A3」可能匹配到集成开发工具的主窗
- 「A3IDE」可能匹配到开发工具的主窗而不是登录窗
- `FindLoginWindow` 没做主窗排除，注释了但代码未实现
- 控件匹配假设简单表单结构（Label + 兄弟 Edit），如果有 GroupBox 嵌套会找不到
...
**需求:** 启动 web 选 Google 这种页面,默认窗口太小看着不舒服,希望启动后窗口最大化。

**实现(`Forms/MainForm.cs` `BuildBrowserArgs`):**

- 加 `maxPart = newWindow ? " --start-maximized" : ""`(仅新窗口模式生效)
- 四个浏览器参数都加:
  - Chrome: `--new-window --start-maximized --no-first-run ...`
  - Edge: `--new-window --start-maximized --user-data-dir=...`
  - Firefox: `-new-window -start-maximized`(语法不同,参数名前缀为短横线)
  - 360: `--new-window --start-maximized`

**Tab 模式说明:** Tab 模式走 explorer.exe 跳板(explorer.exe 打开 URL 让系统默认浏览器在已开实例里开新 Tab),不走 BuildBrowserArgs 也不走 Process.Start,因此无法控制 Tab 是否最大化。但 Tab 是在用户已开 Edge 实例里开, Edge 本身可能已经是最大化的,新 Tab 跟主窗口一致。
...
**问题:** 陛下反馈新窗口模式启动时填表不稳定,出现两个症状:
1. 账号填成空,导致无法登录
2. 密码填到账号位置

**根因:** 原阶段 0 代码用序列:`focus+select 账号框 → Task.Delay(100) → Input.insertText(username) → Task.Delay(200) → focus+select 密码框 → Task.Delay(100) → Input.insertText(password)`
- 100ms delay 不够,React 异步 state 还没同步
- 两次 `Runtime.evaluate(focus+select)` + `Input.insertText` 跳切时序冲突,导致以下两个症状:
  - 【账号填成空】Input.insertText 触发 React onChange,但 select 选中后到下次 input 事件之间,React state 可能被 重置 为空
  - 【密码填到账号位置】focus 切换可能没及时生效,password 被插到还获得焦点的账号框

**修复(`Services/CdpHelper.cs` `AutoLoginAsync`):**

- 抽出 `FillAndVerifyAsync(selJs, value, label)` 本地函数,复用账号/密码填表逻辑
- 每步严格序列:
  1. `Runtime.evaluate(focus + select)` → Task.Delay(300) (焦点稳定)
  2. `Input.insertText(text)` → Task.Delay(500) (等 React state 同步)
  3. `EvaluateAsync` 读 input.value,验证是否等于预期
  4. 不符重试 3 次:清空(focus + value='' + dispatchEvent input)→ 重填
- 账号、密码独立调用 FillAndVerifyAsync,互相不影响
- 去掉原来的【统一 verifyResult】逻辑(不够细),改为每步独立验证
