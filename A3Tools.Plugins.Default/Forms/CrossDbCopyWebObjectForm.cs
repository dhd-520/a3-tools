        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                for (int i = 0; i < codes.Length; i++)
                {
                    ...
                    CopyTableData(srcConn, tgtConn, "S_WEBOBJECT", "GUID", webObjectGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_WEBCONTROL", "WEBOBJECTGUID", webObjectGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_WEBDATA", "WEBOBJECTGUID", webObjectGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_WEBSTYLE", "WEBOBJECTGUID", webObjectGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_WEBCMD", "WEBOBJECTGUID", webObjectGuid, deleteFirst);
                    successCount++;
                }
