        ...
        string[] codes, bool deleteFirst)
    {
        try
        {
            ...
            for (int i = 0; i < codes.Length; i++)
            {
                ...
                var guidObj = await ProxyHelper.ExecuteScalarAsync(srcDA, $"SELECT GUID FROM dbo.S_WEBOBJECT WHERE CODE = '{ProxyHelper.EscapeSql(code)}'");
                var webObjectGuid = guidObj?.ToString();
                ...
                await ProxyHelper.CopyTableDataAsync(srcDA, tgtDA, "S_WEBOBJECT", "GUID", webObjectGuid, deleteFirst, "[Web看板]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_WEBCONTROL", "WEBOBJECTGUID", webObjectGuid, deleteFirst, "[Web看板]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_WEBDATA", "WEBOBJECTGUID", webObjectGuid, deleteFirst, "[Web看板]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_WEBSTYLE", "WEBOBJECTGUID", webObjectGuid, deleteFirst, "[Web看板]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_WEBCMD", "WEBOBJECTGUID", webObjectGuid, deleteFirst, "[Web看板]");
                successCount++;
            }
