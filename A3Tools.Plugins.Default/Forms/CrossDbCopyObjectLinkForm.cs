...
public partial class CrossDbCopyObjectLinkForm : Form
{
    ...
    public CrossDbCopyObjectLinkForm(IToolContext context, Account? currentAccount)
    {
        _context = context;
    ...
    private async void BtnConfirm_Click(object? sender, EventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(txtObjectLinkCode.Text))
        {
            MessageBox.Show("请输入单据流转CODE！", "验证失败", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }

        var codes = txtObjectLinkCode.Text.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries)
            .Select(c => c.Trim()).Where(c => !string.IsNullOrEmpty(c)).ToArray();
        if (codes.Length == 0)
        {
            MessageBox.Show("请输入有效的单据流转CODE！", "验证失败", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        lblProgress.Text = "正在复制单据流转...";
        progressBar.Value = 50;

        var success = await CopyObjectLinksAsync(
        ...
        if (success)
        {
            ...
            //MessageBox.Show("单据流转复制完成！", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
            // 不自动关闭，方便继续操作
        }
        else
        {
    ...
    private async Task<bool> CopyObjectLinksAsync(
        ...
        string[] codes, bool deleteFirst)
    {
        try
        {
            ...
            for (int i = 0; i < codes.Length; i++)
            {
                ...
                var guidObj = await ProxyHelper.ExecuteScalarAsync(srcDA, $"SELECT GUID FROM dbo.S_OBJECTLINK WHERE CODE = '{ProxyHelper.EscapeSql(code)}'");
                var mainGuid = guidObj?.ToString();
                ...
                await ProxyHelper.CopyTableDataAsync(srcDA, tgtDA, "S_OBJECTLINK", "GUID", mainGuid, deleteFirst, "[单据流转]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_OBJECTLINKDETAIL", "OBJECTLINKGUID", mainGuid, deleteFirst, "[单据流转]");
                await ProxyHelper.CopyTableDataByParentGuidAsync(srcDA, tgtDA, "S_OBJECTLINKCROSSCHECK", "OBJECTLINKGUID", mainGuid, deleteFirst, "[单据流转]");
                successCount++;
            }
            ...
            {
                MessageBox.Show($"单据流转复制完成！共 {successCount} 个。", "完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            return true;
        }
        catch (Exception ex)
    ...
    /// <summary>
    /// 输入框内容构造成临时 Account（Http 模式下从 _srcAccount/_tgtAccount 拷贝代理配置）
    /// </summary>
...
ORDER BY NAME";
    ...
    private void BtnAddSelected_Click(object? sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show("请先选择要添加的单据流转！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        // 追加到现有内容
        var currentText = txtObjectLinkCode.Text.Trim();
        ...
        if (newCodes.Count == 0)
        {
            MessageBox.Show("选中的单据流转已全部添加！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }

        var addedText = string.Join(";", newCodes);
        txtObjectLinkCode.Text = currentText + separator + addedText;

        lblSearchProgress.Text = $"已添加 {newCodes.Count} 个单据流转到列表";
        lblSearchProgress.ForeColor = Color.Green;
    }
