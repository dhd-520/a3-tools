...
public partial class SearchBackendForm : Form
{
    ...
    public SearchBackendForm(IToolContext context)
    {
        _context = context;
        InitializeComponent();
        LoadPresetAccount();
    ...
    private void BtnSearch_Click(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var sql = $@"
SELECT A.GUID OBJECTGUID,
       A.CODE 代码,
       A.NAME 名称,
       F.CODE 解决方案代码,
       F.NAME 解决方案,
       A.SUBSYSTEMGUID 业务分组代码,
       B.NAME 业务分组,
       A.OBJECTGROUPGUID 业务分组代码,
       C.NAME 业务分组一,
       D.NAME 业务分组二,
       E.NAME 业务分组三
FROM S_OBJECT A
...
LEFT JOIN S_OBJECTGROUP C ON A.OBJECTGROUPGUID = C.GUID
LEFT JOIN S_OBJECTGROUP D ON C.PARENTGUID = D.GUID
