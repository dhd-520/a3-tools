...
public partial class CrossDbCopyReportForm : Form
{
    ...
    private async Task BtnSearchHttpAsync()
    {
        ...
        try
        {
            var sql = $"SELECT GUID, CODE, NAME, NOTES FROM S_REPORT WHERE CODE LIKE '%{ProxyHelper.EscapeSql(keyword)}%' OR NAME LIKE '%{ProxyHelper.EscapeSql(keyword)}%' ORDER BY NAME";
