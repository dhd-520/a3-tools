...
public partial class CrossDbCopyReportForm : Form
{
    ...
    private async void BtnConfirm_Click(object? sender, EventArgs e)
    {
        ...
        if (string.IsNullOrWhiteSpace(txtReportCode.Text))
        {
            MessageBox.Show("请输入报表CODE！", "验证失败", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }

        var codes = txtReportCode.Text.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries)
            .Select(c => c.Trim()).Where(c => !string.IsNullOrEmpty(c)).ToArray();
        if (codes.Length == 0)
        {
            MessageBox.Show("请输入有效的报表CODE！", "验证失败", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        lblProgress.Text = "正在复制报表...";
        progressBar.Value = 50;

        var success = await CopyReportsAsync(
        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                for (int i = 0; i < codes.Length; i++)
                {
                    ...
                    CopyTableData(srcConn, tgtConn, "S_REPORT", "GUID", reportGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_REPORTDOUBLECLICK", "REPORTGUID", reportGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_REPORTLINKDEFINE", "REPORTGUID", reportGuid, deleteFirst);
                    CopyTableDataByParentGuid(srcConn, tgtConn, "S_REPORTSCHEME", "REPORTGUID", reportGuid, deleteFirst);
                    successCount++;
                }
